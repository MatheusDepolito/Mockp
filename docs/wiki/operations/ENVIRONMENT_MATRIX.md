...
| `apps/web-manager` / `@mockp/web-manager` | `3002` | `apps/web-manager/.env.example` | Same frontend env vars as `apps/web`, with app-specific `NEXTAUTH_URL` | Uses `NEXT_PUBLIC_API_URL` | No direct DB dependency | Status: Current project state | Manager app evidence from `/new-garage`, `/valets`, `/bookings`. |
| `apps/web-valet` / `@mockp/web-valet` | `3003` | `apps/web-valet/.env.example` | Same frontend env vars as `apps/web`, with app-specific `NEXTAUTH_URL` | Uses `NEXT_PUBLIC_API_URL` | No direct DB dependency | Status: Current project state | Valet app evidence from `/my-trips`. |
...
- `apps/web-valet`: `http://localhost:3003`
